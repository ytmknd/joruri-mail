class Gw::WebmailMailbox < ActiveRecord::Base
  ...
  include Gw::Webmail::Mailboxes::Imap
  ...
  def filters
    Gw::WebmailFilter.where(user_id: user_id, mailbox: name)
  end

  def mail_nodes
    Gw::WebmailMailNode.where(user_id: user_id, mailbox: name)
  end
  ...
  def update_filters
    Gw::WebmailFilter.where(user_id: user_id, mailbox: name_was).each do |filter|
      filter.update_columns(mailbox: rename_path(filter.mailbox, name_was, name))
    end
  end

  def update_mail_nodes
    Gw::WebmailMailNode.where(user_id: user_id, mailbox: name_was).update_all(mailbox: name)
  end
