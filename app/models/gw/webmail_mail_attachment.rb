...
class Gw::WebmailMailAttachment < ActiveRecord::Base
  ...
  def save_file(file)
    ...
    total_size = self.size + self.class.where(tmp_id: tmp_id).map{|c| c.size.to_i}.inject(&:+).to_i
    ...
    self.mime_type =
      if (magic = MIME.check_globs(self.name)) || (magic = MIME.check_magics(file.path))
        magic.content_type
      else
