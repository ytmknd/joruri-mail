class Webmail::AddressGroup < ApplicationRecord
  ...
  has_many :children, -> { order(:name, :id) },
    foreign_key: :parent_id, class_name: 'Webmail::AddressGroup', dependent: :destroy
  has_many :groupings, foreign_key: :group_id, class_name: 'Webmail::AddressGrouping', dependent: :destroy
  ...
  scope :addresses_counts, -> {
    joins(:addresses).group(:id).count('webmail_addresses.id')
  }
