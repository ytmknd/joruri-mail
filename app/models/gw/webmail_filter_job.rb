class Gw::WebmailFilterJob
  ...
  def perform
    ...
    init_core(user) do
      filters = Gw::WebmailFilter.where(user_id: user.id, state: 'enabled').order(:sort_no, :id)
        .preload(:conditions)
      Gw::WebmailFilter.apply_uids(filters, mailbox: mailbox, uids: uids)
    end
  end
