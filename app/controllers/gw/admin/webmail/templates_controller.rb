class Gw::Admin::Webmail::TemplatesController < Gw::Controller::Admin::Base
  ...
  def index
    @items = Gw::WebmailTemplate.readable.where(user_id: Core.user.id).order(:name, :id)
      .paginate(page: params[:page], per_page: params[:limit])
    _index @items
  end

  def show
    @item = Gw::WebmailTemplate.find(params[:id])
  ...
  def new
    @item = Gw::WebmailTemplate.new(default_flag: 0)
  end

  def create
    @item = Gw::WebmailTemplate.new(item_params)
