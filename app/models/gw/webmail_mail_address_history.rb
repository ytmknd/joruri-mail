class Gw::WebmailMailAddressHistory < ActiveRecord::Base
  ...
  class << self
    ...
    def load_address_map(emails)
      addrs = Gw::WebmailAddress.where(email: emails, user_id: Core.current_user.id).pluck(:email, :name).flatten
      Hash[*addrs]
    end

    def load_sys_address_map(emails)
      addrs = Sys::User.where(email: emails).pluck(:email, :name).flatten
      Hash[*addrs]
    end
  end
end
